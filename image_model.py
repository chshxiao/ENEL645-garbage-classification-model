...
class GarbageModel(nn.Module):
  ...
  def forward(self, x):
    x = self.pretrained_model(x)                        # vgg - add a new layer
    x = self.classifier(x)                              # vgg - add a new layer
    return x                                            # vgg - add a new layer
...
"""
def image_model_predict(num_classes, path, test_loader):
  # get the model
  test_model = GarbageModel(num_classes=num_classes, transfer=False)
